...
df = df.drop(['s0','s1','s2','s3','s4'],axis=1)
...
X_train = scaler.fit_transform(df_train.drop(['time_different'],axis=1).as_matrix())
y_train = scaler.fit_transform(df_train['time_different'].as_matrix().reshape(-1, 1))
#y_train = df_train['Close'].as_matrix()

X_test = scaler.fit_transform(df_test.drop(['time_different'],axis=1).as_matrix())
y_test = scaler.fit_transform(df_test['time_different'].as_matrix().reshape(-1, 1))
#y_test = df_test['Close'].as_matrix()
print(X_train.shape)
print(np.max(y_test),np.max(y_train),np.min(y_test),np.min(y_train))

def denormalize(df,norm_data):
    df = df['time_different'].values.reshape(-1,1)
...
output,W_O = neural_net_model(xs,4)
